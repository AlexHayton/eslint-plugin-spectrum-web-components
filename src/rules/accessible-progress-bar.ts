...
const createRule = ESLintUtils.RuleCreator(
  (name) =>
    `https://github.com/AlexHayton/eslint-plugin-spectrum-web-components-lit/blob/main/docs/rules/${name}.md`,
);

type MessageIds = "missingLabel";

export const accessibleProgressBar = createRule<[], MessageIds>({
  name: "accessible-progress-bar",
  meta: {
    type: "suggestion",
    docs: {
      description: "Require accessible labeling on <sp-progress-bar> elements",
    },
    messages: {
      missingLabel:
        "<sp-progress-bar> needs one of the following to be accessible: a `label` attribute, an `aria-label` attribute, or an `aria-labelledby` attribute.",
    },
    schema: [],
  },
  defaultOptions: [],
  create(context) {
    function checkTemplate(node: TSESTree.TaggedTemplateExpression) {
      ...
      const elements = extractElements(templateText, "sp-progress-bar");
